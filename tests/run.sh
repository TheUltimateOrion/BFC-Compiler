...
"$compiler" -S "$root_dir/tests/hello.bf" -o "$temp_dir/hello.s"
cmp "$root_dir/tests/hello.bf.s" "$temp_dir/hello.s"

"$compiler" -S "$root_dir/tests/multiply.bf" -o "$temp_dir/multiply.s"
test -s "$temp_dir/multiply.s"

printf '[+\n' > "$temp_dir/malformed.bf"
if "$compiler" -S "$temp_dir/malformed.bf" -o "$temp_dir/malformed.s" \
